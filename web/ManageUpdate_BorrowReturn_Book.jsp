...
<html>
    <head>
        ...
        <title>KẾT QUẢ CẬP NHẬT PHIÊN TRẢ SÁCH</title>
    ...
    <body>
        <c:if test="${requestScope.errorMessage!=null}">
            <h1>CẬP NHẬT PHIÊN TRẢ SÁCH THẤT BẠI</h1>
            <h3 class="center">${requestScope.errorMessage}</h3>
        </c:if>  
        <c:if test="${requestScope.successMessage!=null}">
            <h1>KẾT QUẢ CẬP NHẬT PHIÊN TRẢ SÁCH</h1>
            <h3 class="center">*Lưu ý: Phiên trả sách đã được cập nhật trước đó sẽ KHÔNG được cập nhật lại.<br>KHÔNG cho phép cập nhật hoặc hiển thị phiên trả sách nếu dữ liệu không hợp lệ.</h3>
            <div>
                <table>
                    <tr>
                    ...
                    <c:forEach var="list" items="${requestScope.returnBookRecentUpdate}">
                        <tr class="border-rds">
                            <td>${list.borrowId}</td>
                            <td>${list.borrowDate}</td>
                            <td>
                                <c:if test="${list.returnStatus=='Done'}">Đã trả</c:if>
                                <c:if test="${list.returnStatus=='Not yet'}">Chưa trả</c:if>
                                </td>
                            ...
                            <td>
                                ...
                                <td>
                                <c:if test="${list.bookSttAfter=='New'}">Mới</c:if>
                                <c:if test="${list.bookSttAfter=='Good'}">Tốt</c:if>
                                <c:if test="${list.bookSttAfter=='Lower'}">Trung bình</c:if>
                                <c:if test="${list.bookSttAfter=='Worst'}">Kém</c:if>
                                </td>
                                <td>
